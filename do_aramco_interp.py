...
for i,eb in enumerate(ebs):
    ...
    for j in range(1510,1601,10):
        ...
        comm="sz_interp %s -3 449 449 235 %f>interp_aramco.txt" % (filepath,eb)
        os.system(comm)
       
        with open("interp_aramco.txt","r") as f:
        ...
        os.system("rm -f interp_aramco.txt")


np.savetxt("interpres/sz_aramco_cr.txt",cr,delimiter='\t')
np.savetxt("interpres/sz_aramco_psnr.txt",psnr,delimiter='\t')
np.savetxt("interpres/sz_aramco_ctime.txt",ctime,delimiter='\t')
np.savetxt("interpres/sz_aramco_dtime.txt",dtime,delimiter='\t')