...
for j,idx in enumerate(idxrange):
    for i,eb in enumerate(ebs):
        ...
        if(compress_mode!=2 or i==0):
            if compress_mode!=5:
                comm="python3 predict.py -sp 256 -c %s -k %s -i %s -d 3 -e %f -l %sl.dat -r %sr.dat -s %d -p 1 -x 512 -y 512 -z 512 -mx %f -mi %f -eps %f -para 1 -v 1 >%s_t1.txt" % (configpath,ckptpath,filepath,latent_eb,pid,pid,blocksize,maximum[field],minimum[field],eps,pid)
            else:
