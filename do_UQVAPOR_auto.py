...
for i,eb in enumerate(ebs):
    ...
    for j in range(41,49):
        ...
        comm="sz_autotuning %s 100 500 500 %f>auto_%s.txt" % (filepath,eb,field)
        os.system(comm)
...
np.savetxt("autores/sz_%s_cr.txt" % field,cr,delimiter='\t')
np.savetxt("autores/sz_%s_psnr.txt" % field,psnr,delimiter='\t')
np.savetxt("autores/sz_%s_ctime.txt" % field,ctime,delimiter='\t')
np.savetxt("autores/sz_%s_dtime.txt" % field,dtime,delimiter='\t')